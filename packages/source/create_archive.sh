...
if test -f ${DSTDIR}/CMakeLists.txt
  then
    sed -e "s/set(CMAKE_PACKAGE_VERSION \"dev\")/set(CMAKE_PACKAGE_VERSION \"${TAG}\")/" -ibak ${DSTDIR}/CMakeLists.txt
fi
sed -ibak -e "s/AC_INIT.*/AC_INIT(crrcsim, ${TAG}, crrcsim-devel@lists.sourceforge.net)/" configure.ac
sed -ibak -e "s/.define PRODUCT_VERSION .*/!define PRODUCT_VERSION \"${TAG}\"/" packages/Win32/crrcsim.nsi
